...
/**
 * Opens a beacon's menu from an inventory slot index.
 *
 ...
 */
public final class BeaconMenuOpener {

    /** Only the 36 slots the menu actually draws. A beacon in the offhand still works, but the menu
    ...
    /**
     * The beacon the key binding should open: the one worn as a curio if there is one, otherwise the
     * first in the inventory, hotbar first.
     *
     * <p>Curios is checked first to match which beacon actually runs. Before this the binding only
     * ever looked in the inventory, so a player wearing their only beacon pressed the key and nothing
     * happened at all - which reads as the binding being broken rather than as a missed slot.
     */
    public static int findBeacon(ServerPlayer player) {
        if (!CuriosCompat.findBeacon(player).isEmpty()) {
    ...
    /** Returned by {@link #findBeacon} when the player has no beacon at all. */
    public static final int NONE = -99;

    private BeaconMenuOpener() {}
}
