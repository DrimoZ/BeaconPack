...
public class PortableBeaconMenu extends AbstractContainerMenu {
    ...
    public PortableBeaconMenu(int containerId, Inventory playerInventory, int packSlotIndex) {
    ...
    public PackState state() {
        return PortableBeaconItem.stateOf(pack());
    }
    ...
    public PackTierDef tierDef() {
        if (!(pack().getItem() instanceof PortableBeaconItem item)) {
            return null;
        }
    ...
    public boolean applyAction(int action, int slotIndex, int value) {
        ...
        PortableBeaconItem.setState(pack(), PackResolver.sanitize(updated, stats, lookup, tier));
